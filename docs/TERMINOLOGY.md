...
- [ ] **`relay_mcp_session_id`** = session merge key; tool returns JSON `{relay_mcp_session_id, human, cmd_skill_count}` (optional `attachments`, optional `data_url` per item from MCP); pass id on next call; if **`cmd_skill_count` is 0**, next call must repass commands+skills; tab label **MM-DD HH:mm:ss** (**RELAY_MCP_SESSION_ID.md**)
