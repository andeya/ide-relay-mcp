...
- [ ] **`relay_mcp_session_id`** = session merge key; tool returns JSON `{relay_mcp_session_id, human, cmd_skill_count}` (optional `attachments` `{kind, path}`; MCP may rewrite paths for WSL when `RELAY_EXE_IN_WSL`); pass id on next call; if **`cmd_skill_count` is 0**, next call must repass commands+skills; tab label **MM-DD HH:mm:ss** (**RELAY_MCP_SESSION_ID.md**)
