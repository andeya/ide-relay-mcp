...
- **正常**：JSON \`{ "relay_mcp_session_id": "<ms>", "human": "<用户回答>", "cmd_skill_count": <number> [, "attachments": [{ "kind": "image"|"file", "path": "...", "data_url"?: "<data URL>" }] ] }\`；无附件时省略 \`attachments\`。\`path\` 始终保留；\`relay mcp\` 可对**小文件**额外写入 \`data_url\`（见 \`RELAY_MCP_INLINE_ATTACHMENTS\` / \`RELAY_MCP_INLINE_MAX_BYTES\`）。\`cmd_skill_count\` = 当前该标签页已保存的 **commands + skills** 条数（ slash 补全列表大小）。
