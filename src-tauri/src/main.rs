...
fn read_dragged_image_preview(path: String) -> Result<DraggedImagePreview, String> {
    ...
    // Drag-drop image previews are base64-encoded for the webview thumbnail;
    // 25 MB is intentionally stricter than MAX_ATTACH_BYTES (50 MB) to keep
    // in-memory base64 overhead manageable for thumbnails.
...
fn run_tauri(initial: LaunchState) {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            system_ui::setup_system_tray(app)?;
            system_ui::setup_macos_dock_menu();
            Ok(())
        })
        ...
        .unwrap_or_else(|e| {
            eprintln!("relay: failed to build Tauri app: {e}");
            std::process::exit(1);
        });
    app.run(|app, event| {
        #[cfg(target_os = "macos")]
