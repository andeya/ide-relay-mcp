...
#[cfg(target_os = "macos")]
fn setup_macos_dock_menu(is_zh: bool) {
    ...
    extern "C" {
        ...
        fn objc_msgSend();
        fn class_addMethod(cls: Cls, sel: Sel, imp: *const std::ffi::c_void, types: *const std::ffi::c_char) -> bool;
    ...
    unsafe fn nsstr(s: &str) -> Id {
        ...
        msg1(clz(b"NSString\0"), sel(b"stringWithUTF8String:\0"), c.as_ptr() as Id)
    }
    ...
    unsafe {
        ...
        let label = if is_zh { "\u{65b0}\u{5efa}\u{7a97}\u{53e3}" } else { "New Window" };
