...
- **桥接** — 每次交互前 MCP 读取端点文件；若缺失或不健康，**以参数 `gui` 拉起同一可执行文件**，轮询至多 **~45 s**（`ensure_gui_endpoint`）。随后 **`POST /v1/feedback`** → **`GET /v1/feedback/wait/:request_id`**（长轮询，默认 **60 分钟 s** / 客户端约 **700 s**）。响应为 **JSON** `{relay_mcp_session_id, human}`，即工具返回值。
